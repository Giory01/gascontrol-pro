...
// Componentes, Estilos y el Hook de Inactividad
...
const DashboardPage = () => {
    ...
    // Lógica para filtrar y resumir pedidos
    const pedidosVisibles = useMemo(() => {
        return pedidos.filter(pedido => pedido.estado?.toLowerCase() !== 'cancelado');
    }, [pedidos]);
        ...
        <>
            ...
            <Navbar className="dashboard-navbar mb-4">
                <Container fluid>
                    ...
                    <Navbar.Collapse className="justify-content-end">
                        <img src={currentUser.photoURL || `https://ui-avatars.com/api/?name=${currentUser.displayName || 'G'}&background=0d47a1&color=fff`} alt="perfil" width="30" height="30" className="d-inline-block align-top rounded-circle me-3" />
            ...
            <Container>
                ...
                <div className="pedidos-list">
                    {loading ? (
                        ...
                        <Card className="text-center p-4"><Card.Body><Card.Title>No tienes pedidos activos</Card.Title><Card.Text>Haz clic en "+ Nuevo Pedido" para empezar.</Card.Text></Card.Body></Card>
                    ) : (
                        pedidosVisibles.map(pedido => (
                            <Card key={pedido.id} className={`pedido-card estado-${pedido.estado?.toLowerCase().replace(' ', '')}`}>
                                ...
                                <div className="d-flex justify-content-between align-items-center mt-2 pt-2 border-top">
                                    ...
                                    <Badge bg={pedido.tipoPago === 'Fiado' ? 'danger' : 'primary'}>
                                        {pedido.tipoPago}
                                    </Badge>
                                </div>
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)} centered>
                <Modal.Header closeButton>
                    <Modal.Title>Registrar Nuevo Pedido</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form onSubmit={handleCrearPedido}>
                        ...
                        <Row>
                            <Col>
                                <Form.Group className="mb-3">
                                    ...
                                    <Form.Select value={tamanoTanque} onChange={e => {
                                        setTamanoTanque(e.target.value);
                                        calcularPrecioTotalModal(e.target.value, numeroDeTanques);
                                    }}>
                            ...
                            <Col>
                                <Form.Group className="mb-3">
                                    ...
                                    <Form.Control type="number" value={numeroDeTanques} onChange={e => {
                                        setNumeroDeTanques(e.target.value);
                                        calcularPrecioTotalModal(tamanoTanque, e.target.value);
                                    }} min="1" required />
                                </Form.Group>
                            </Col>
                        </Row>
                        <Form.Group className="mb-3">
                            ...
                            <div>
                                <Form.Check
                                    inline type="radio" label="Contado" name="tipoPago" id="pagoContado" value="Contado"
                                    checked={tipoPago === 'Contado'} onChange={(e) => setTipoPago(e.target.value)}
                                />
                                <Form.Check
                                    inline type="radio" label="Fiado" name="tipoPago" id="pagoFiado" value="Fiado"
                                    checked={tipoPago === 'Fiado'} onChange={(e) => setTipoPago(e.target.value)}
                                />
                            </div>
                        </Form.Group>
                        <div className="mt-3 text-end">
                            <h5 className="fw-bold">Total a Pagar: <span className="text-success">${precioTotalModal.toFixed(2)}</span></h5>
                        </div>
